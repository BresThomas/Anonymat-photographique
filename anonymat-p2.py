    ...
    if len(orders["shapes"]) == 0:
        ...
        for shape in orders["shapes"]:
            if shape["type"] == "circle":
                ...
                print(f"  Rectangle de coins ({shape['c1x']}, {shape['c1y']}) et ({shape['c2x']}, {shape['c2y']})")

                # Calculer la couleur moyenne du rectangle dans 'im_in'
                average_color = average_color_rectangle(im_in, (shape["c1x"], shape["c1y"]), (shape["c2x"], shape["c2y"]))

                # Appliquer cette couleur au rectangle dans 'im_out'
                fill_rectangle(im_out, (shape["c1x"], shape["c1y"]), (shape["c2x"], shape["c2y"]), average_color)
